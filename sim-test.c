...
int test_savers(unsigned int rseed) {
    ...
    GSC_GLOBALX_T firstparents[] = {2, 3, 1, 4, 0};
    GSC_GLOBALX_T secondparents[] = {4, 1, 3, 2, 5};
    ...
    GSC_GLOBALX_T toprint[] = {0,1,//2,3,4,5 from g0
                     6,7,//8,9,10 from f1
    ...
    char* fname = "t1.txt";
    save_genotypes(fname,d,NO_GROUP,GSC_FALSE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_allele_matrix)==0);
    remove(fname);

    fname = "t2.txt";
    save_genotypes(fname,d,NO_GROUP,GSC_TRUE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_transposed_allele_matrix)==0);
    remove(fname);

    fname = "t3.txt";
    save_genotypes(fname,d,printingGroup,GSC_FALSE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_group_alleles)==0);
    remove(fname);

    fname = "t4.txt";
    save_genotypes(fname,d,printingGroup,GSC_TRUE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_transposed_group_alleles)==0);
    remove(fname);
    ...
    fname = "t1b.txt";
    save_bvs(fname,d,NO_GROUP,effSet1);
    assert(compareFileToString(fname, TEST1_TRUTH_save_bvs)==0);
    remove(fname);

    fname = "t2b.txt";
    save_bvs(fname,d,printingGroup,effSet1);
    assert(compareFileToString(fname, TEST1_TRUTH_save_group_bvs)==0);
    remove(fname);
    ...
    fname = "t1c.txt";
    save_markerblocks(fname,d,exampleMB,NO_MAP);
    assert(compareFileToString(fname, TEST1_TRUTH_save_marker_blocks_blocksonly)==0);
    remove(fname);

    fname = "t2c.txt";
    save_markerblocks(fname,d,exampleMB,(MapID){.id=1});
    ...
    fname = "t1d.txt";
    save_pedigrees(fname,d,NO_GROUP,GSC_FALSE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_one_step_pedigrees)==0);
    remove(fname);


    fname = "t2d.txt";
    save_pedigrees(fname,d,printingGroup,GSC_FALSE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_group_one_step_pedigrees)==0);
    remove(fname);

    fname = "t3d.txt";
    save_pedigrees(fname,d,NO_GROUP,GSC_TRUE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_full_pedigrees)==0);
    remove(fname);

    fname = "t4d.txt";
    save_pedigrees(fname,d,printingGroup,GSC_TRUE);
    assert(compareFileToString(fname, TEST1_TRUTH_save_group_full_pedigrees)==0);
    remove(fname);


    // try save-as-you-go savers (ideally you'd run this with a very low CONTIG_WIDTH, and also for all crossing funcs.
    GSC_GLOBALX_T parentix[] = {20};
...
static void check_mapfile(SimData* d, char* filename, int it) {
    ...
    assert(strcmp(d->genome.marker_names[0],"3rd") == 0);
    assert(strcmp(d->genome.marker_names[1],"fourth") == 0);
    assert(strcmp(d->genome.marker_names[2],"5") == 0);
    assert(strcmp(d->genome.marker_names[3],"first") == 0);
    assert(strcmp(d->genome.marker_names[4],"second") == 0);
	...
	assert(strcmp(d->genome.maps[it-1].chr_names[0],"1A") == 0);
	assert(strcmp(d->genome.maps[it-1].chr_names[1],"3") == 0);
    assert(d->genome.maps[it-1].chrs[1].type == GSC_LINKAGEGROUP_SIMPLE);
    assert(d->genome.maps[it-1].chrs[0].type == GSC_LINKAGEGROUP_REORDER);
    assert(d->genome.maps[it-1].chrs[1].map.simple.n_markers == 2);
    assert(fabs(d->genome.maps[it-1].chrs[1].map.simple.expected_n_crossovers - 0.4) < TOL);
    assert(fabs(d->genome.maps[it-1].chrs[1].map.simple.dists[0] - 0) < TOL);
    assert(fabs(d->genome.maps[it-1].chrs[1].map.simple.dists[1] - 1) < TOL);
    assert(d->genome.maps[it-1].chrs[1].map.simple.first_marker_index == 3);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.n_markers == 3);
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.expected_n_crossovers - 0.1) < TOL);
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.dists[0] - 0) < TOL);
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.dists[1] - 0.83) < TOL);
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.dists[2] - 1) < TOL);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.marker_indexes[0] == 2);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.marker_indexes[1] == 0);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.marker_indexes[2] == 1);

    SimData* d2 = create_empty_simdata(3);
    assert(load_mapfile(d2, filename).id == 1);

    assert(d2->genome.n_markers == 5);
	assert(strcmp(d2->genome.marker_names[0],"5") == 0);
	assert(strcmp(d2->genome.marker_names[1],"3rd") == 0);
    assert(strcmp(d2->genome.marker_names[2],"fourth") == 0);
    assert(strcmp(d2->genome.marker_names[3],"first") == 0);
    assert(strcmp(d2->genome.marker_names[4],"second") == 0);
	...
	assert(strcmp(d2->genome.maps[0].chr_names[0],"1A") == 0);
	assert(strcmp(d2->genome.maps[0].chr_names[1],"3") == 0);
    assert(d2->genome.maps[0].chrs[0].type == GSC_LINKAGEGROUP_SIMPLE);
    assert(d2->genome.maps[0].chrs[1].type == GSC_LINKAGEGROUP_SIMPLE);
    assert(d2->genome.maps[0].chrs[1].map.simple.n_markers == 2);
    assert(fabs(d2->genome.maps[0].chrs[1].map.simple.expected_n_crossovers - 0.4) < TOL);
    assert(fabs(d2->genome.maps[0].chrs[1].map.simple.dists[0] - 0) < TOL);
    assert(fabs(d2->genome.maps[0].chrs[1].map.simple.dists[1] - 1) < TOL);
    assert(d2->genome.maps[0].chrs[1].map.simple.first_marker_index == 3);
    assert(d2->genome.maps[0].chrs[0].map.simple.n_markers == 3);
    assert(fabs(d2->genome.maps[0].chrs[0].map.simple.expected_n_crossovers - 0.1) < TOL);
    assert(fabs(d2->genome.maps[0].chrs[0].map.simple.dists[0] - 0) < TOL);
    assert(fabs(d2->genome.maps[0].chrs[0].map.simple.dists[1] - 0.83) < TOL);
    assert(fabs(d2->genome.maps[0].chrs[0].map.simple.dists[2] - 1) < TOL);
    ...
    assert(d2->genome.maps[nmaps-1].chrs[0].map.reorder.marker_indexes[0] == 3);
    assert(d2->genome.maps[nmaps-1].chrs[0].map.reorder.marker_indexes[1] == 0);
    assert(d2->genome.maps[nmaps-1].chrs[0].map.reorder.marker_indexes[2] == 4);
    ...
    assert(fabs(d2->genome.maps[nmaps-1].chrs[1].map.simple.expected_n_crossovers - 0) < TOL);
    assert(fabs(d2->genome.maps[nmaps-1].chrs[2].map.simple.expected_n_crossovers - 0) < TOL);
    assert(d2->genome.maps[nmaps-1].chrs[0].map.simple.first_marker_index == 3);
    assert(d2->genome.maps[nmaps-1].chrs[1].map.simple.first_marker_index == 0);
    assert(d2->genome.maps[nmaps-1].chrs[2].map.simple.first_marker_index == 4);
...
int test_maploaders2(void) {
    ...
    assert(d->genome.maps[it-1].n_chr == 2);
	assert(strcmp(d->genome.maps[it-1].chr_names[0],"1B") == 0);
	assert(strcmp(d->genome.maps[it-1].chr_names[1],"3") == 0);
    assert(d->genome.maps[it-1].chrs[0].type == GSC_LINKAGEGROUP_REORDER);
    assert(d->genome.maps[it-1].chrs[1].type == GSC_LINKAGEGROUP_REORDER);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.n_markers == 2);
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.expected_n_crossovers - 0.4) < TOL);
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.dists[0] - 0) < TOL);
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.dists[1] - 1) < TOL);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.marker_indexes[0] == 4);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.marker_indexes[1] == 3);
    assert(d->genome.maps[it-1].chrs[1].map.reorder.n_markers == 2);
    assert(fabs(d->genome.maps[it-1].chrs[1].map.reorder.expected_n_crossovers - 0.083) < TOL);
    ...
    assert(fabs(d->genome.maps[it-1].chrs[0].map.reorder.dists[1] - 1) < TOL);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.marker_indexes[0] == 2);
    assert(d->genome.maps[it-1].chrs[0].map.reorder.marker_indexes[1] == 1);
    remove(filename);
    ...
    assert(d->genome.maps[it-1].chrs[0].map.simple.first_marker_index == 4);
    remove(filename);
...
static void check_matrix_with_different_specification_levels(int seed, char* filename, const char* mapfile, int only_full,
    ...
    if (!only_full) {
        ...
        spec.spec.matrix.markers_as_rows = GSC_NA;
        spec.spec.matrix.cell_style = GSC_GENOTYPECELLSTYLE_UNKNOWN;
        load_genotypefile(d2,filename,spec);
        ++it; check_genotypes(d2,it,0);

        // only orientation
        spec.spec.matrix.has_header = GSC_NA;
...
m2 AT AA TT
    */
    ...
    f1 = TEST1_GENOMATRIX_LOADERS[it-1];
    write_to_file(filename, f1);
    load_genotypefile(d2,filename,define_matrix_format_details(GSC_FALSE,GSC_NA,GSC_GENOTYPECELLSTYLE_UNKNOWN));
    cid += 3;
    fflush(stdout);
    assert(d2->n_groups == 1);
    assert(d2->current_id.id == cid);
    ...
    f1 = TEST1_GENOMATRIX_LOADERS[it-1];
    write_to_file(filename, f1);
    load_genotypefile(d2,filename,define_matrix_format_details(GSC_NA,GSC_FALSE,GSC_GENOTYPECELLSTYLE_UNKNOWN));
    cid += 3;
    fflush(stdout);
    assert(d2->n_groups == 1);
    assert(d2->current_id.id == cid);
...
GroupNum test_labels(SimData *d, GroupNum g0) {
    ...
    // & repeat, without specific choice of group
    const int newestlabel1values[] = {2,4,6};
    change_label_to_values(d, NO_GROUP, 2, label1, 3, newestlabel1values);
    assert(d->n_labels == 2);
    assert(d->m->n_labels == 2);
    assert(d->m->labels != NULL);
    assert(d->m->labels[label1index] != NULL);
    ...
    assert(d->m->labels[label1index][0] == newlabel1value);
    assert(d->m->labels[label1index][1] == newerlabel1values[0]);
    assert(d->m->labels[label1index][2] == newestlabel1values[0]);
    assert(d->m->labels[label1index][3] == newestlabel1values[1]);
    assert(d->m->labels[label1index][4] == newestlabel1values[2]);
    assert(d->m->labels[label1index][5] == newlabel1value);
    const int fixlabel1values[] = {newerlabel1values[1],newerlabel1values[2],newerlabel1values[3],newlabel1value};
    change_label_to_values(d, g0, 2, label1, 4, fixlabel1values); // fix for further tests
    assert(d->m->labels[label1index][1] == newerlabel1values[0]);
    assert(d->m->labels[label1index][2] == newerlabel1values[1]);
    assert(d->m->labels[label1index][3] == newerlabel1values[2]);
    assert(d->m->labels[label1index][4] == newerlabel1values[3]);
...
GroupNum test_random_splits(SimData *d, GroupNum g0) {
    ...
    for (int i = 0; i < 10; ++i) {
        GroupNum grpB[3];
        GSC_GLOBALX_T grpBsizes[3] = {1,3,2};
...
GroupNum test_specific_splits(SimData *d, GroupNum g0) {
    ...
    GSC_GLOBALX_T combinations[2][4];
    ...
    GSC_GLOBALX_T g0members[6] = {0,1,2,3,4,5};
...
int test_local_effect_calculators(RND_U32 seed) {
    ...
    MarkerBlocks b2 = create_evenlength_blocks_each_chr(d, init.map, 2);
    assert(b2.num_blocks == 6);
    assert(b2.num_markers_in_block[0] == 1);
    assert(b2.num_markers_in_block[1] == 1);
    assert(b2.num_markers_in_block[2] == 2);
    assert(b2.num_markers_in_block[3] == 1);
    assert(b2.num_markers_in_block[4] == 1);
    assert(b2.num_markers_in_block[5] == 0);
    assert(b2.markers_in_block[0][0] == 0);
    assert(b2.markers_in_block[1][0] == 1);
    assert(b2.markers_in_block[2][0] == 2);
    assert(b2.markers_in_block[2][1] == 3);
    assert(b2.markers_in_block[3][0] == 4);
    assert(b2.markers_in_block[4][0] == 5);
    delete_markerblocks(&b2);
    
    b2 = create_evenlength_blocks_each_chr(d, init.map, 5);
    assert(b2.num_blocks == 3*5);
    assert(b2.num_markers_in_block[0] == 1);
    assert(b2.num_markers_in_block[1] == 0);
    assert(b2.num_markers_in_block[2] == 0);
    assert(b2.num_markers_in_block[3] == 0);
    assert(b2.num_markers_in_block[4] == 1);
    assert(b2.num_markers_in_block[5] == 1);
    assert(b2.num_markers_in_block[6] == 1);
    assert(b2.num_markers_in_block[7] == 0);
    assert(b2.num_markers_in_block[8] == 0);
    assert(b2.num_markers_in_block[9] == 1);
    assert(b2.num_markers_in_block[10] == 1);
    assert(b2.num_markers_in_block[11] == 0);
    assert(b2.num_markers_in_block[12] == 0);
    assert(b2.num_markers_in_block[13] == 0);
    assert(b2.num_markers_in_block[14] == 0);
    assert(b2.markers_in_block[0][0] == 0);
    assert(b2.markers_in_block[4][0] == 1);
    assert(b2.markers_in_block[5][0] == 2);
    assert(b2.markers_in_block[6][0] == 3);
...
int test_crossing_randomly(SimData *d, GroupNum g1) {
    ...
    GSC_GLOBALX_T g2ixs[4];
    assert(get_group_indexes(d, g2, 0, g2ixs) == 4);

    assert(get_group_size(d, g2) == 4);
    GSC_ID_T g2minid = d->m->ids[g2ixs[0]].id;
    GSC_ID_T g2maxid = d->m->ids[g2ixs[3]].id;
    fprintf(stdout, "Should be random parents: %d, %d, %d, %d\n",
            (int) d->m->pedigrees[0][g2ixs[0]].id, (int) d->m->pedigrees[0][g2ixs[1]].id,
            (int) d->m->pedigrees[0][g2ixs[2]].id, (int) d->m->pedigrees[0][g2ixs[3]].id);
    fprintf(stdout, "Should be random parents: %d, %d, %d, %d\n\n",
            (int) d->m->pedigrees[1][g2ixs[0]].id, (int) d->m->pedigrees[1][g2ixs[1]].id,
            (int) d->m->pedigrees[1][g2ixs[2]].id, (int) d->m->pedigrees[1][g2ixs[3]].id);
    ...
    fprintf(stdout, "Should be random parents: %d, %d, %d\n",
            (int) d->m->pedigrees[0][g2ixs[3] + 1].id, (int) d->m->pedigrees[0][g2ixs[3] + 3].id,
            (int) d->m->pedigrees[0][g2ixs[3] + 5].id);
    fprintf(stdout, "Should be random parents: %d, %d, %d\n",
            (int) d->m->pedigrees[1][g2ixs[3] + 1].id, (int) d->m->pedigrees[1][g2ixs[3] + 3].id,
            (int) d->m->pedigrees[1][g2ixs[3] + 5].id);
...
int test_deletors(SimData *d, GroupNum g0) {
    ...
    GSC_GLOBALX_T groupcounts1b[1000];
...
int test_getters(SimData* d, GroupNum gp) {
    ...
    char* names[6];
    assert(get_group_names(d, gp, 0, names) == 6);
    ...
    PedigreeID ids[10];
    assert(get_group_ids(d, gp, -1, ids) == 6); // one old-style form using -1 to mean unknown group size.
    for (int i = 0; i < 6; ++i) {
        assert(ids[i].id == i+1);
    }

    GSC_GLOBALX_T indexes[6];
    ...
    assert(get_group_parent_ids(d, f1, 0, 1, p1s) == 3);
    ...
    assert(get_group_parent_names(d, f1, 0, 2, p2ns) == 3);
