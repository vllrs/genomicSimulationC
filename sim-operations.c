...
AlleleMatrix* create_empty_allelematrix(const int n_markers, const int n_labels, const int labelDefaults[n_labels], const int n_genotypes) {
    ...
    if (n_labels > 0) {
        ...
        fprintf(stderr, "Invalid negative number of labels provided to create_empty_allelematrix");
        m->labels = NULL;
    }
...
LabelID create_new_label(SimData* d, const int setTo) {
    ...
    do {
        ...
        // Consider the case we need to initialise the label list
            ...
            fprintf(stderr, "Unable to create new label for all genotypes; SimData may be corrupted.\n");
            warned = TRUE;
        }
...
/** Comparator function for qsort. Used to compare an array of size_t to sort
 * them in ascending order.
...
unsigned int _split_by_somequality( SimData* d, const GroupNum group_id,  
        ...
        size_t maxentries_results, GroupNum results[maxentries_results]) {
...
GroupNum split_evenly_into_two(SimData* d, const GroupNum group_id) {
    ...
    size_t size = get_group_size(d, group_id);
    if (size < 2) {
        if (size < 1) {
