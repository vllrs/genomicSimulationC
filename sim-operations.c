...
void gsc_change_label_default(gsc_SimData* d, const gsc_LabelID whichLabel, const int newDefault) {
    ...
    if (whichLabel.id == GSC_NO_LABEL.id || (labelIndex = gsc_get_index_of_label(d, whichLabel)) < 0) {
        fprintf(stderr, "Nonexistent label %d\n", whichLabel.id);
        return;
    }
...
void gsc_change_allele_symbol(gsc_SimData* d, const char* which_marker, char from, char to) {
    ...
    if (which_marker == NULL) {
        ...
        while (IS_VALID_LOCATION(loc)) {
            for (size_t m = 0; m < d->genome.n_markers; ++m) {
    ...
    printf("Changed allele %c to %c %u times across %u markers and %u genotypes\n", from, to, nalleles, nmarkers, ngenos);
}
...
void gsc_move_genotype(gsc_GenoLocation from, gsc_GenoLocation to, int* label_defaults) {
    ...
    if (to.localAM->groups[to.localPos].num != GSC_NO_GROUP.num) {
        fprintf(stderr,"In moving a genotype from %p:%d to %p:%d, the genotype at %p:%d will be overwritten\n", from.localAM, from.localPos, to.localAM, to.localPos, to.localAM, to.localPos);
    ...
    if (to.localAM->n_labels != from.localAM->n_labels) {
        fprintf(stderr,"Origin and destination when copying genotype do not have the same number of custom labels (n_labels). The genotype now at %p:%d will have lost its label data.\n", to.localAM, to.localPos);
    } else {
 ...
 */
gsc_GenoLocation gsc_next_get_nth(gsc_RandomAccessIterator* it, const size_t n) {
...
gsc_GroupNum gsc_make_group_from( gsc_SimData* d, const int index_list_len, const size_t* genotype_indexes) {
    if (index_list_len < 1) {
        fprintf(stderr,"Invalid index_list_len value: length of allocation list must be positive.\n");
        return GSC_NO_GROUP;
    }
    ...
    if (invalidLocations > 0) {
        fprintf(stderr,"%d indexes were invalid.\n",invalidLocations);
    }
...
unsigned int gsc_scaffold_split_by_somequality( gsc_SimData* d, const gsc_GroupNum group_id,  
    ...
    GSC_CREATE_BUFFER(currentsizes,size_t,d->n_groups);
...
gsc_GroupNum gsc_split_evenly_into_two(gsc_SimData* d, const gsc_GroupNum group_id) {
    ...
    size_t size = gsc_get_group_size(d, group_id);
    if (size < 2) {
        if (size < 1) {
            fprintf(stderr,"Group %d does not exist\n", group_id.num);
        } else {
            fprintf(stderr,"Group %d has only one member so can't be split\n", group_id.num);
        }
        return GSC_NO_GROUP;
    }

    size_t even_half = size / 2;
    GSC_CREATE_BUFFER(allocations,size_t,size);
...
static gsc_GroupNum gsc_helper_split_by_allocator_knowncounts(gsc_GenoLocation loc, gsc_SimData* d, void* datastore, unsigned int n_outgroups,
    ...
    *subgroupsfound = n_outgroups;
	unsigned int randpos = rnd_pcg_range(&d->rng,0,cumulative_counts[n_outgroups-1] - 1);
...
unsigned int gsc_split_evenly_into_n(gsc_SimData* d, const gsc_GroupNum group_id, const int n, gsc_GroupNum* results) {
    if (n <= 1) {
        fprintf(stderr, "Invalid n value: number of fractions into which to split group must be positive.\n");
		return 0;
	}
...
static gsc_GroupNum gsc_helper_split_by_allocator_equalprob(gsc_GenoLocation loc, gsc_SimData* d, void* datastore, unsigned int n_outgroups,
                                             unsigned int* subgroupsfound, gsc_GroupNum* outgroups) {
	int randgroup = rnd_pcg_range(&d->rng,0,n_outgroups-1);
...
unsigned int gsc_split_randomly_into_n(gsc_SimData* d, const gsc_GroupNum group_id, const int n, gsc_GroupNum* results) {
    if (n <= 1) {
        fprintf(stderr, "Invalid n value: number of fractions in which to split group must be positive.\n");
        return 0;
    }
 ...
 */
int gsc_get_existing_group_counts( gsc_SimData* d, gsc_GroupNum* out_groups, size_t* out_sizes) {
 ...
 * if group size was set; equal to the actual size of the group if `group_size` was -1.
 */
int gsc_get_group_indexes(const gsc_SimData* d, const gsc_GroupNum group_id, int group_size, size_t* output) {
    const gsc_AlleleMatrix* m = d->m;
    if (group_size <= 0) { // group_size == GSC_UNINIT
        group_size = gsc_get_group_size( d, group_id );
        if (group_size == 0) { return 0; }
...
void gsc_delete_recombination_map(gsc_SimData* d, const gsc_MapID which_map) {
    ...
    if (which_map.id == GSC_NO_LABEL.id || (map_ix = gsc_get_index_of_map(d, which_map)) < 0) {
        fprintf(stderr, "Nonexistent recombination map %d\n", which_map.id);
        return;
    }
 ...
 */
int gsc_get_index_of_genetic_marker(const char* target, gsc_KnownGenome g, size_t* out) {
...
static int gsc_helper_parse_3cell_header(gsc_TableFileReader* tf, const char** canonical_titles, int* col_order,
                                          gsc_TableFileCell* unprocessedqueue, size_t* queuesize) {
    const int ncells = 3;
 ...
 */
static size_t gsc_helper_str_markerlist_leftjoin( gsc_KnownGenome g, size_t n_markers_in_list, struct gsc_MapfileUnit** markerlist ) {
    ...
    for (size_t i = 0; i < n_markers_in_list; ++i) {
 ...
 */
static void gsc_helper_sort_markerlist(size_t n_markers, struct gsc_MapfileUnit* markerlist) {
 ...
 */
gsc_MapID gsc_create_recombmap_from_markerlist(gsc_SimData* d, size_t n_markers, struct gsc_MapfileUnit* markerlist) {
    if (n_markers == 0) return NO_MAP;

    GSC_CREATE_BUFFER(chr_nmembers,size_t,40);
    ...
    for (int chr_ix = 0; chr_ix < map.n_chr; ++chr_ix) {		
        ...
        size_t firsts_coord_in_genome = d->genome.n_markers; 
        size_t n_goodmembers = 0;
        size_t* marker_coords = NULL;

        size_t endpt = first_marker + chr_nmembers[chr_ix];
        for (; current_marker < endpt; ++current_marker) { // simple recombination map, if possible
            ...
            if (!found_first) {
                ...
                for (size_t backfill = 0; backfill < n_goodmembers; ++backfill) {
 ...
 * @return gsc_MapID of the gsc_RecombinationMap that was just loaded into the simulation.
 */
gsc_MapID gsc_create_uniformspaced_recombmap(gsc_SimData* d, size_t n_markers, char** markernames, double expected_n_recombinations) {
    ...
    if (markernames == NULL) {
        ...
        for (size_t i = 0; i < d->genome.n_markers; ++i) { lgdists[i] = lgdist; }
        ...
        char found_first = GSC_FALSE;
        size_t could_not_match;
        size_t firsts_coord_in_genome = d->genome.n_markers;
        size_t chrmarker_ix = 0;
        ...
        for (size_t i = 0; i < n_markers; ++i) {
            if (!found_first || marker_coords != NULL) {
                ...
                for (size_t backfill = 0; backfill < chrmarker_ix; ++backfill) {
                    marker_coords[backfill] = firsts_coord_in_genome + backfill;
                }
        ...
        for (size_t i = 0; i < chrmarker_ix; ++i) { lgdists[i] = lgdist; }
...
gsc_MapID gsc_load_mapfile(SimData* d, const char* filename) {
    ...
    if (d->genome.n_markers > 0) {
        ...
        size_t new_nrows = gsc_helper_str_markerlist_leftjoin(d->genome, nrows, &mapcontents);
        if (new_nrows < nrows) {
            printf("Discarded %u markers when loading map %s because they do not appear in the primary map.\n", (unsigned int) (nrows - new_nrows), filename);
        }
        nrows = new_nrows;
...
gsc_EffectID gsc_load_effectfile(gsc_SimData* d, const char* filename) {
    ...
    int goodrow = (header) ? GSC_FALSE : GSC_TRUE; // discard first row if it's a header, keep if it's not.
    size_t goodrow_counter = 0;
    size_t allele_counter = 0;

    GSC_CREATE_BUFFER(effset_alleles,char,2);
    GSC_CREATE_BUFFER(effset_rows,double*,2);
    size_t n_effset_rows = 0;
	
    size_t markerix;
    char allele = '\0';
    size_t alleleix = n_effset_rows + 1; // invalid value. 
    double effect = 0;
    char* conversionflag;
 ...
 */
static void gsc_helper_genotypecell_to_allelematrix(GenoLocation loc, size_t markerix,
...
static struct gsc_GenotypeFile_MatrixFormat gsc_helper_genotypefile_matrix_detect_orientation(const SimData* d, 
        const gsc_TableFileCell* cellqueue, const size_t firstrowlen, const size_t queuelen,
...
static struct gsc_GenotypeFile_MatrixFormat gsc_helper_genotypefile_matrix_detect_cellstyle(const gsc_TableFileCell* cellqueue, 
        const size_t firstrowlen, const size_t queuelen, struct gsc_GenotypeFile_MatrixFormat format, const char* filenameforlog) {
 ...
 */
static int gsc_helper_genotypefile_matrix_detect_cornercell_presence(const size_t ncellsfirstrow, const size_t ncellssecondrow, const int secondrowheaderisempty) {
...
static gsc_GroupNum gsc_load_genotypefile_matrix(gsc_SimData* d, const char* filename, const gsc_FileFormatSpec format) {
    ...
    if (d->genome.n_markers == 0) {
        if (format_detected.markers_as_rows) {
            ...
            size_t nmarkersread = format_detected.has_header ? 0 : 1;
            do {
    ...
    if (format_detected.markers_as_rows) {
        ...
        int first = GSC_TRUE; 
        int have_valid_marker = GSC_FALSE; size_t markerix;
        ...
        size_t* markerixs = gsc_malloc_wrap(sizeof(*markerixs)*ncellsfirstrow,GSC_TRUE);
        for (size_t j = 0; i < ncellsfirstrow; ++i, ++j) {
    ...
    for (int chr = 0; chr < d->genome.maps[map_index].n_chr; ++chr) {
        ...
        switch (map.chrs[chr].type) {
            ...
            case GSC_LINKAGEGROUP_REORDER:
                ...
                break;
            default:
                fprintf(stderr, "Linkage group type of group with index %i of map with index %ui is corrupted.\n", chr, map_index);
				num_crossovers = 0;
        }
...
gsc_GroupNum gsc_make_random_crosses(gsc_SimData* d, 
	...
	if (g_size == 0) {
        ...
        fprintf(stderr,"Group %d must contain multiple individuals to be able to perform random crossing\n", from_group.num);
        return GSC_NO_GROUP;
    }
    if (d->genome.n_maps < 1) {
...
gsc_GroupNum gsc_make_n_crosses_from_top_m_percent(gsc_SimData* d, const int n, const int m, const gsc_GroupNum group,
    ...
    if (effIndex == GSC_UNINIT) {
        fprintf(stderr,"Cannot find marker effect set with id %d\n", effID.id);
        return GSC_NO_GROUP;
    }
    if (m < 1 || m > 100) {
...
gsc_GroupNum gsc_split_by_bv(gsc_SimData* d, const gsc_GroupNum group, const gsc_EffectID effID, const int top_n, const int lowIsBest) {
    ...
    GSC_CREATE_BUFFER(top_individuals,size_t,top_n);
...
gsc_DecimalMatrix gsc_calculate_utility_bvs(gsc_BidirectionalIterator* targets, const gsc_EffectMatrix* effset) {
    ...
    size_t n_markers = effset->effects.cols;
    ...
    size_t i = 0; // highest allele index
...
gsc_DecimalMatrix gsc_calculate_allele_counts(const gsc_SimData* d, const gsc_GroupNum group, const char allele) {
    ...
    size_t n_markers = d->m->n_markers;
    gsc_DecimalMatrix counts = gsc_generate_zero_dmatrix(n_genotypes, n_markers );
                ...
                for (int i = 0; i < map.chrs[chr].map.simple.n_markers; ++i) {
                    ...
                    while (current_block_filling < n - 1 && chrpos > current_block_filling / n) {
                        ...
                        if (blocks.num_markers_in_block[b] > 0) {
                            size_t bcapacity = sizeof(*blocks.markers_in_block[b])*blocks.num_markers_in_block[b];
...
void gsc_calculate_group_local_bvs(const gsc_SimData* d, const gsc_MarkerBlocks b, const gsc_EffectID effID, const char* output_file, const gsc_GroupNum group) {
    const int effIndex = gsc_get_index_of_eff_set(d, effID);
    if (effIndex == GSC_UNINIT) {
        fprintf(stderr,"Nonexistent effect set with id %d\n", effID.id);
        return;
    }
    gsc_EffectMatrix e = d->e[effIndex];
...
double gsc_calculate_optimal_possible_bv(const gsc_SimData* d, const gsc_GroupNum group, const gsc_EffectID effID) {
    ...
    int gsize = gsc_get_group_size(d, group);
	if (gsize == 0) {
		fprintf(stderr,"Nonexistent group %d\n", group.num);
        return 0;
	}
    GSC_CREATE_BUFFER(ggenes,char*,gsize);
    gsc_get_group_genes(d, group, gsize, ggenes);
...
void gsc_save_bvs(const char* fname, const gsc_SimData* d, const gsc_GroupNum groupID, 
    ...
    if (effix == GSC_UNINIT) {
        fprintf(stderr, "Marker effect set %i does not exist: cannot calculate breeding values\n", effID.id); return;
    }
...
static int gsc_helper_is_marker_in_chr(const size_t markerix, const gsc_LinkageGroup chr, double* pos) {
    ...
    switch (chr.type) {
        ...
        case GSC_LINKAGEGROUP_REORDER:
            for (size_t i = 0; i < chr.map.reorder.n_markers; ++i) {
...
void gsc_save_utility_markerblocks(FILE* f, const gsc_MarkerBlocks b, const size_t n_markers, 
	...
	for (size_t i = 0; i < b.num_blocks; ++i) {
            ...
            if (b.num_markers_in_block[i] > 0) {
                ...
                for (int chrix = 0; chrix < map->n_chr; ++chrix) {
                    if (gsc_helper_is_marker_in_chr(b.markers_in_block[i][0],
                        ...
                        isonchr = chrix;
                        maxpos = minpos;
                        for (size_t j = 1; j < b.num_markers_in_block[i]; ++j) {
                            double pos;
        ...
        for (size_t j = 0; j < b.num_markers_in_block[i]; ++j) {
			size_t k = b.markers_in_block[i][j];
			if (k <= n_markers) {
...
static void gsc_scaffold_save_genotype_info(FILE* f, gsc_BidirectionalIterator* targets, 
        size_t n_markers, char** const marker_names, const int markers_as_rows,
        void (*bodycell_printer)(FILE*, gsc_GenoLocation, size_t, void*), void* bodycell_printer_data) {
        ...
        case GSC_FALSE:
            ...
            if (targets != NULL) {
                ...
                while (IS_VALID_LOCATION(loc)) {
                    ...
                    // Row body
                    for (size_t i = 0; i < targets->cachedAM->n_markers; ++i) {
                        fwrite("\t", sizeof(char), 1, f);
...
static void gsc_helper_output_genotypematrix_cell(FILE* f, gsc_GenoLocation loc, size_t markerix, void* NA) {
...
static void gsc_helper_output_countmatrix_cell(FILE* f, gsc_GenoLocation loc, size_t markerix, void* data) {
    if (IS_VALID_LOCATION(loc)) {
...
void gsc_save_utility_allele_counts(FILE* f, gsc_BidirectionalIterator* targets,
		size_t n_markers, char** const marker_names, const int markers_as_rows, const char allele) {
    gsc_scaffold_save_genotype_info(f, targets, n_markers, marker_names, markers_as_rows, 
...
static void gsc_scaffold_save_ancestry_of(const gsc_AlleleMatrix* m, gsc_PedigreeID p1, gsc_PedigreeID p2,
        void (*strprinter)(char*, size_t, void*), void (*intprinter)(int, void*), void* printer_data) {
