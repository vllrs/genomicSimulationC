...
static GSC_LOGICVAL gsc_helper_parse_ncell_header(int ncellrow1,
    ...
    for (; title_ix < ncell_required; ++title_ix) {
        ...
        _Bool found_match = 0;
        size_t title_len = strlen(titles_required[title_ix]);
    ...
    for (; title_ix < ncell_total; ++title_ix) {
        ...
        size_t title_len = strlen(titles_optional[title_ix_o]);
...
gsc_MapID gsc_load_mapfile(SimData* d, const char* filename) {
	...
	for (size_t i = 0; i < nrows; ++i) { // free the chr names that haven't been moved to the proper map object
